...
function _escapeSingleQuote(string){
...
var trimFn = 'function _trim(input, pattern){ ' +
    ...
    '}';
var fn = '(function fn(){ return function(m){ ~whereFn~ ~setFn~ ~trimFn~ var __var = {stmt: \'\', params: [], loopCtxt: []};' +
    '~replacement~ return {stmt: __var.stmt, params: __var.params, type: \'~queryType~\'}; }; })()';
        ...
        switch(node.type){
            ...
            case 'where':
                if(queryType !== 'insert'){
                    if(start){
                        ...
                        rplc += ' __var.stmt += (whereResult.trim()) ? ( \' WHERE \' + whereResult ) : \'\'; ';
                        context = '';
                    }
                }
                break;
            case 'set':
                if(queryType === 'update'){
                    if(start){
                        ...
                        rplc += ' __var.stmt += (setResult.trim()) ? ( \' SET \' + setResult ) : \'\'; ';
                        context = '';
                    }
                }
                break;
            case 'trim':
                if(start){
                    ...
                    rplc += ' __var.stmt += (trimResult.trim()) ? trimResult : \'\'; ';
                    context = '';
                }
                break;
            case 'if':
            ...
            case 'foreach':
                ...
                if(start){
                    ...
                    rplc += ' ' + (context ? context + 'Clause' : '__var.stmt') + ' += ' + options.open + ';';
                    rplc += ' var close = ' + options.close + ';';
                    rplc += ' for(var i=0; i<' + options.collection + '.length; i++){ ';
                    rplc += '   var ' + options.item + ' = ' + options.collection + '[i];';
                    rplc += ' if(i > 0){' + (context ? context + 'Clause' : '__var.stmt') + ' += ' + options.sep + '; }';
                    evalContext = '';
                } else {
                    evalContext = 'm';
                    rplc += ' } ';
                    rplc += ' ' + (context ? context + 'Clause' : '__var.stmt') + ' += close;';
                }
                break;
            case 'text':
                if (context) {
                    rplc += ' ' + context + 'Clause += \'' + _escapeSingleQuote(node.content) + ' \'; ';
                } else {
                    rplc += ' __var.stmt += \'' + _escapeSingleQuote(node.content) + ' \'; ';
                }
                break;
            case 'placeholder':
                if (context) {
                ...
                rplc += ' __var.params.push(' + _evalNormalize(node.evaluation, evalContext) + '); ';
                break;
        }
        //console.log(node);
    });
...
/*function _test(){
    var testStr = [
        ...
        {#foreach collection=outer, item=i, open=\'(\', close=\')\', sep=\',\'}\
            {#foreach collection=inner, item=j, sep=\',\'}\
                {{j.value}}\
            {/foreach}\
        {/foreach}'
    ];
